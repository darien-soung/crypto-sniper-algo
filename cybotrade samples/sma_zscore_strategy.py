    ...
    async def on_candle_closed(self, strategy, topic, symbol):
        ...
        if z_score > self.z_score_threshold:
            if current_pos.long.quantity == 0.0:
                try:
                    await strategy.open(exchange=Exchange.BybitLinear, side=OrderSide.Buy, quantity=self.quantity,
                                        symbol=symbol, limit=None, take_profit=None, stop_loss=None,
                                        is_hedge_mode=self.hedge_mode, is_post_only=False)
                except Exception as e:
                    logging.error(f"Failed to open long: {e}")
        else:
            if current_pos.long.quantity != 0.0:
                try:
                    await strategy.close(exchange=Exchange.BybitLinear, side=OrderSide.Buy, quantity=self.quantity,
                                         symbol=symbol, is_hedge_mode=self.hedge_mode)
                except Exception as e:
